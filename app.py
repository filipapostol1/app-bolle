...
def init_db():
    """Inizializza il database con la tabella utenti e la tabella cronologia."""
    ...
    # Tabella Cronologia collegata all'ID Utente
    cursor.execute("""
...
def hash_password(password):
    """Crea un hash SHA-256 sicuro della password."""
...
def carica_cronologia(user_id):
    """Carica la cronologia SOLO per l'utente collegato."""
    init_db()
    conn = sqlite3.connect(DB_NAME)
...
def salva_cronologia(user_id, azione, cliente, dettaglio, importo="-"):
    """Salva una nuova operazione legata all'ID dell'utente attivo."""
    init_db()
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute(
...
def pulisci(testo):
    """Pulisce i testi per evitare crash dei PDF."""
    if testo is None:
        return ""
...
def crea_pdf_bolla(dati):
    ...
    pdf.cell(
        184,
        4,
        pulisci("PRESCRIZIONI PARTICOLARI DEL COMMITTENTE / ISTRUZIONI:"),
        ln=1,
    )
    ...
    istruzioni_extra = (
        ...
        else "Nessuna istruzione particolare specificata."
    )
...
if not st.session_state["autenticato"]:
    ...
    scelta_modalita = st.radio(
        "Scegli un'opzione:",
        ["Accedi", "Registra Nuova Azienda"],
        horizontal=True,
    )
    ...
    with col2:
        if scelta_modalita == "Accedi":
            ...
            pass_reg_confirm = st.text_input(
...
# --- APP PRINCIPALE (DOPO IL LOGIN) ---
    ...
    with tab1:
        ...
        with st.form("form_bolla"):
            ...
            c4, c5 = st.columns(2)
            vettore = c4.text_input(
                "Nome Tua Azienda (Vettore)",
            ...
            note_committente = st.text_input(
                ...
                "Consegna entro le ore 14:00",
            )

            invia_bolla = st.form_submit_button(
                "Genera Bolla PDF", type="primary"
            )
    ...
    with tab3:
        ...
        if not df_storico.empty:
            ...
            st.info(
                "Nessun documento nel tuo archivio. Genera un preventivo o una bolla per iniziare!"
            )
